...
export default function App() {
  const [step,        setStep]        = useState(1); // 1=config, 2=generating, 3=done, 4=error
  ...
  const applyPaste = (tid) => {
    ...
    if (!players.length) { setPasteErr(e=>({...e,[tid]:"No valid rows. Copy directly from Excel or Google Sheets."})); return; }
    setPasteErr(e=>({...e,[tid]: players.length>16 ? `Only first 16 of ${players.length} imported.` : ""}));
    ...
    try {
      const res = await fetch("/api/create-sheet", {
        ...
        body: JSON.stringify({
          ...
          startDate,
          email,
          days: days.map(d=>({label:d.label, games:d.games})),
          teams: teams.map(t=>({
            name: t.name||"Team",
            league: t.league,
  ...
  }, [tourneyName, startDate, email, days, teams]);
  ...
  const card = {
    background:"rgba(255,255,255,0.035)", border:"1px solid rgba(200,168,75,0.2)",
    borderRadius:10, padding:"18px", marginBottom:14,
  };
  const sec = {
    background:"rgba(255,255,255,0.04)", border:"1px solid rgba(200,168,75,0.25)",
    borderRadius:12, padding:"22px", marginTop:20,
  };
  const secTitle = {
    fontSize:"0.88rem", fontWeight:"bold", color:"#c8a84b",
    textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:14,
  };
  const btn = (v="gold") => ({
    padding: v==="sm" ? "5px 12px" : "11px 24px",
    borderRadius:6, border:"none", cursor:"pointer", fontWeight:"bold",
    fontSize: v==="sm" ? "0.76rem" : "0.88rem", letterSpacing:"0.05em",
    background: v==="danger" ? "rgba(180,50,50,0.7)"
      : v==="ghost" ? "rgba(255,255,255,0.07)"
  ...
  if (step===3) return (
    <div style={{minHeight:"100vh",background:"#0a1628",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:20,fontFamily:"'Georgia',serif",padding:24,textAlign:"center"}}>
      ...
      <div style={{color:"#8fa8c8",fontSize:"0.9rem",maxWidth:440,lineHeight:1.7}}>
        ...
        Check your inbox for an invite, or open it directly below.
      </div>
      <a href={sheetUrl} target="_blank" rel="noreferrer" style={{
        display:"inline-block", marginTop:8,
        padding:"14px 36px", borderRadius:8,
        background:"linear-gradient(135deg,#c8a84b,#a07830)",
        color:"#0a1628", fontWeight:"bold", fontSize:"1rem",
        textDecoration:"none", letterSpacing:"0.06em",
      }}>⚾ Open Spreadsheet</a>
      <button style={{...btn("ghost"), marginTop:8}} onClick={()=>setStep(1)}>
        ← Create Another
      </button>
    </div>
  );
  ...
  if (step===4) return (
    <div style={{minHeight:"100vh",background:"#0a1628",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:20,fontFamily:"'Georgia',serif",padding:24,textAlign:"center"}}>
      ...
      <div style={{color:"#8fa8c8",fontSize:"0.88rem",maxWidth:440,background:"rgba(200,80,80,0.1)",border:"1px solid rgba(200,80,80,0.3)",borderRadius:8,padding:"12px 16px"}}>
        {errMsg}
      </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0a1628",fontFamily:"'Georgia',serif",color:"#e8dcc8",paddingBottom:80}}>
      ...
      <div style={{
        ...
        borderBottom:"3px solid #c8a84b", padding:"36px 32px 28px", textAlign:"center",
      }}>
        <div style={{fontSize:"2.2rem",marginBottom:6}}>⚾</div>
        <h1 style={{margin:"0 0 6px",fontSize:"2rem",fontWeight:"bold",color:"#c8a84b",letterSpacing:"0.1em",textTransform:"uppercase"}}>
          Tournament Pitcher Tracker
        </h1>
        <p style={{margin:0,color:"#8fa8c8",fontSize:"0.88rem"}}>
          Fill in your details — we'll build the Google Sheet and send it straight to your inbox.
        </p>
      </div>
      ...
      <div style={{maxWidth:980,margin:"0 auto",padding:"0 20px"}}>
        ...
        {/* Tournament Info */}
        <div style={sec}>
          <div style={secTitle}>⚙️ Tournament Details</div>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:12,flexWrap:"wrap"}}>
            ...
            <div>
              ...
              <div style={{fontSize:"0.7rem",color:"#5a7a9a",marginTop:3}}>Sets season year for league age calculation</div>
            </div>
            <div>
              ...
              <div style={{fontSize:"0.7rem",color:"#5a7a9a",marginTop:3}}>Sheet will be shared to this address</div>
            </div>
          </div>
        </div>
        ...
        {/* Days */}
        <div style={sec}>
          <div style={secTitle}>📅 Tournament Days</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:8}}>
            {days.map((d,i) => (
              <div key={d.id} style={{background:"rgba(200,168,75,0.06)",border:"1px solid rgba(200,168,75,0.2)",borderRadius:8,padding:"10px 14px",display:"flex",alignItems:"center",gap:10}}>
                ...
                <div style={{width:110}}>
                  <select style={sel} value={d.games} onChange={e=>updateDay(i,"games",Number(e.target.value))}>
        ...
        <div style={sec}>
          ...
          {teams.map((team,ti) => (
            <div key={team.id} style={card}>
              {/* Team header */}
              <div style={{display:"grid",gridTemplateColumns:"2fr 1fr auto",gap:10,marginBottom:12,alignItems:"end"}}>
                <div>
                ...
                <div>
                  ...
                  <select style={sel} value={team.league} onChange={e=>updateTeam(team.id,"league",e.target.value)}>
                    {LEAGUES.map(l=><option key={l} value={l}>{LEAGUE_NAMES[l]} ({l})</option>)}
                  </select>
                </div>
                <div>
                  {teams.length > 1 && <button style={{...btn("danger"),padding:"8px 12px"}} onClick={()=>removeTeam(team.id)}>✕</button>}
                </div>
              </div>
              ...
              {pasteOpen[team.id] && (
                <div style={{background:"rgba(200,168,75,0.06)",border:"1px solid rgba(200,168,75,0.2)",borderRadius:8,padding:"12px",marginBottom:12}}>
                  <div style={{fontSize:"0.76rem",color:"#8fa8c8",marginBottom:8,lineHeight:1.6}}>
                    Copy rows from Excel or Google Sheets:&nbsp;
                    <strong style={{color:"#c8a84b"}}>Surname · First Name · # · Date of Birth</strong>
                    <br/><span style={{color:"#f08060"}}>Replaces current roster for this team.</span>
                  </div>
